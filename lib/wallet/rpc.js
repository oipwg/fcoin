...
class RPC extends RPCBase {
  ...
  async listUnspent(args, help) {
    ...
    const includeUnsafe = valid.bool(3, true);
    const options = valid.obj(4, { "minimumAmount": 0, "maximumCount": 250 });
    const height = this.wdb.state.height;
    ...
    for (const coin of coins) {
      ...
      if (Amount.btc(coin.value, true) < options.minimumAmount)
        continue;

      if (out.length >= options.maximumCount)
        continue;
